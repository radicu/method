...
def read_csv_files(directory):
    files = ['task.csv', 'task_report.csv', 'task_data.csv', 'task_train.csv']
    dataframes = {}
    for file in files:
...
def main():
    ...
    if dir_name in st.session_state.results:
        ...
        st.info(f"**Train MAE: {data['mae_train']:.2f} | Validation MAE: {data['mae_val']:.2f}**")
        
        test_projects = data['test_data']
        test_projects['Date'] = pd.to_datetime(test_projects['Date']).dt.date
        pred_data = data['pred_data']
        test_projects_ids = test_projects['ProjectID'].unique()
        
        if len(test_projects_ids) > 0:
            test_projects_select = st.sidebar.selectbox('Test Project ID :', test_projects_ids, help='select project ID from the test set')
            test_projects_select_data = test_projects[test_projects['ProjectID'] == test_projects_select]
            
            # Add date filter
            available_dates = test_projects_select_data['Date']
            selected_date = st.sidebar.date_input("Select Date", min_value=available_dates.min(), max_value=available_dates.max(), help='select a date from available dates in the project')
                
            st.sidebar.info(f'Valid date range is from {available_dates.min()} to {available_dates.max()}')
            test_projects_select_data_filtered = test_projects_select_data[test_projects_select_data['Date'] == selected_date]
            test_tasks = test_projects_select_data_filtered['ID'].unique().tolist()
            test_tasks_select = st.sidebar.selectbox('Test Tasks ID :', test_tasks, help='select task ID from the project')
            task_select_data = test_projects_select_data[(test_projects_select_data['ID'] == test_tasks_select) & (test_projects_select_data['Date'] <= selected_date)].reset_index(drop=False)
            ...
            ax.scatter(task_select_data['Date'], task_select_data['Progress'], label='Progress', s=100)
            ax.axvline(pd.to_datetime(task_select_data['EndDate']).iloc[0], color='blue', linestyle='--', label='Planned end date')
            if len(task_select_data['Progress'].unique())>1:
                # Fit line
                slope, intercept, r_value, p_value, std_err = stats.linregress(task_select_data.index, task_select_data['Progress'])
                line = slope * task_select_data.index + intercept
                ax.plot(task_select_data['Date'], line, color='red', label='Fit Line')

                # Error area
                residuals = task_select_data['Progress'] - (slope * task_select_data.index + intercept)
                std_residuals = np.std(residuals)
                ax.fill_between(task_select_data['Date'], line - std_residuals, line + std_residuals, color='red', alpha=0.2, label='Error Area')

                # Predict the date when progress achieves 100
                date_100 = pd.to_datetime((100 - intercept) / slope, unit='D', origin=task_select_data['Date'].min())
